import { Employee, allDesignations, allDepartments } from '@/data/employees';
import { Users, TrendingUp, Hotel, Star, ArrowUpRight, Trophy, Building2, GraduationCap } from 'lucide-react';
...
export default function DashboardStats({ employees }: DashboardStatsProps) {
  ...
  const top3ByDesig = allDesignations
    .filter(d => employees.some(e => e.designation === d))
    .slice(0, 8)
    .map(designation => {
  ...
  return (
    <motion.div className="space-y-8" variants={container} initial="hidden" animate="show">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <motion.div variants={item}>
          <StatCard icon={<Building2 className="h-5 w-5" />} label="Hotels" value={uniqueHotels} />
        </motion.div>
        <motion.div variants={item}>
      ...
      <motion.div variants={item}>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Avg Experience', value: `${Math.round(employees.reduce((s, e) => s + e.experience, 0) / totalEmployees)} yrs` },
            { label: 'Departments', value: `${allDepartments.length}` },
            { label: 'Score ≥ 70', value: `${employees.filter(e => e.finalScore >= 70).length}` },
            { label: 'Needs Improvement', value: `${employees.filter(e => e.finalScore < 40).length}` },
          ].map(stat => (
      ...
      {/* Top 3 per designation */}
      <motion.div variants={item}>
        <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
          <Trophy className="h-4 w-4 text-primary" />
          Top Performers by Designation
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {top3ByDesig.map(({ designation, top3 }) => (
            <motion.div
              ...
              whileHover={{ y: -2 }}
            >
              <p className="text-xs font-bold text-primary uppercase tracking-wider mb-3 truncate">{designation}</p>
