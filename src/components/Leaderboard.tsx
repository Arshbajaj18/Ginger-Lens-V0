        ...
        {ranked.slice(3).map((emp, idx) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="h-3 w-3" />{emp.hotelCity} · {emp.experience} yrs</p>
            </div>
            <Progress value={emp.finalScore} className="w-20 h-1.5 hidden sm:block" />
            <span className={`font-bold text-sm ${emp.finalScore >= 75 ? 'text-success' : emp.finalScore >= 50 ? 'text-foreground' : 'text-destructive'}`}>
              {emp.finalScore}
            </span>
          </motion.div>
        ))}
